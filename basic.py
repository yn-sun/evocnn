...
pool[2,2,0.60],
pool[2,2,0.18],
full[1076,0.25822218285383824,0.6626911711480307],
full[10,0.0670992643760644,0.6910514760392503]
'''
...
def model():
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        conv1 =slim.conv2d(x, 23, [7,7], weights_initializer=tf.truncated_normal_initializer(mean=0.18, stddev=0.87))
        pool1 = slim.avg_pool2d(conv1, [2,2], stride=2, padding='SAME')
        pool2 = slim.max_pool2d(pool1, [2,2], stride=2, padding='SAME')
        flatten = slim.flatten(pool2)
        full1 = slim.fully_connected(flatten, 1076, weights_initializer=tf.truncated_normal_initializer(mean=0.258222, stddev=0.662691), biases_initializer=tf.constant_initializer(0.1, dtype=tf.float32))
        logits = slim.fully_connected(full1, 10, activation_fn=None, weights_initializer=tf.truncated_normal_initializer(mean=0.0670, stddev=0.691), biases_initializer=tf.constant_initializer(0.1, dtype=tf.float32))
