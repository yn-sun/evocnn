...
'''
Length:5, Num:8534644, Mean:0.91, Std:0.04,
conv[8,8,5,-0.10,0.65],
conv[8,8,27,0.17,0.46],
pool[2,2,0.79],
full[1609,-0.0023846697741753964,0.08991355789211397],
full[10,0.425395045663272,0.5545578772968163]
'''
...
def model():
    ...
    with slim.arg_scope([slim.conv2d, slim.fully_connected],
        ...
        conv1 =slim.conv2d(x, 5, [8,8], weights_initializer=tf.truncated_normal_initializer(mean=-0.10, stddev=0.65))
        conv2 =slim.conv2d(conv1, 27, [8,8], weights_initializer=tf.truncated_normal_initializer(mean=0.17, stddev=0.46))
        pool1 = slim.avg_pool2d(conv2, [2,2], stride=2, padding='SAME')
        flatten = slim.flatten(pool1)
        full1 = slim.fully_connected(flatten, 1609, weights_initializer=tf.truncated_normal_initializer(mean=-0.002384, stddev=0.08991), biases_initializer=tf.constant_initializer(0.1, dtype=tf.float32))
        logits = slim.fully_connected(full1, 10, activation_fn=None, weights_initializer=tf.truncated_normal_initializer(mean=0.425395, stddev=0.5545578), biases_initializer=tf.constant_initializer(0.1, dtype=tf.float32))
